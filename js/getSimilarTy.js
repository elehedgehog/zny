...
export const getSimilarTy = (tsid,lon=2,lat=2,angle=30,anglediff=30,speed=30,speeddiff=30,strength=30) => {
  ...
  // const url = `http://10.148.83.228:8881/roa1080/discrete/typhoon/d10/${tsid},${new Date().Format('yyyy-MM-dd HH:00:00')},${lon},${lat},${angle},${anglediff},${speed},${speeddiff},${strength}/JSON`;
  $.ajax({ url }).then(data => {
    ...
    typhoon.map(info => {
      info.replace(/(\d*),(\d*),(.*?),/, function($0,$1,$2,$3) {
        ...
        prArr.push(new Promise((resolve, reject) => {
          $.ajax({ url: `http://119.29.102.103:8111/roa1080/discrete/typhoon/d2/${tyid},BCGZ/JSON?cacheCtrl=${Date.now()}`})      //从该链接获取相似台风名称
          .then(res => {
...
$('#simTyphoon').on('click', '.similarTyInner', function() {
  ...
  if($(this).hasClass('simSelected')) {
    ...
    // let url = `http://119.29.102.103:8111/typhoon/d2?tsid=${tyid}&fcid=BCGZ&cacheCtrl=${Date.now()}`;
    // let url = `http://10.148.83.228:8881/typhoon/d2?tsid=${tyid}&fcid=BCGZ`;
    $.ajax({ url }).then((data) => {
